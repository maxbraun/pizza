...
function analyseFlour(input) {
  ...
  return {
    ...
    // What to push into the calculator's own controls. Protein is only
    // clamped, not snapped to the slider's 0.5 grid: typed entry accepts
    // off-grid values and the flour catalogue is full of them (12.7, 14.2),
    // so snapping would round a real bag figure onto a different flour.
    // plVal is null unless the bag actually printed a P/L — the slider is
    // this app's own extensible↔elastic abstraction, not a printed ratio,
    // so with nothing to go on the honest move is to leave it where the
    // user (or their picked flour) already set it.
    suggest: {
      protein: clamp(protein, 8, 15),
      plVal: plGiven != null ? plSlider : null,
